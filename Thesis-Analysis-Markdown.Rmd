...
#### Descriptive Norms (contact) - 2nd factor (Spouse & Children) - Correlation
```{r, eval=T, echo=F}
...
### CIM - Costs
**It's not valid to do PCA on categorical variables, right? I either need to look at other methods to reduce complexity across these items in the CIM Costs group, or keep them as is.**   
...
d_pca_CIMcosts <- d_accept %>% 
  ...
  dplyr::select(
    starts_with("age"),
    matches("gender"),
    matches("sr_57i_dependents"),
    matches("sr_57j_adultchildren")    
...
pca1 <- fa(d_pca_CIMcosts, nfactors = 1, rotate = "Varimax", fm="minres") 
print(pca1)
print(pca1$loadings,cutoff = 0.3)
```
...
Three-factor solution:   
1. Multi-item composite variable: All types (fed, state, business, ngo, elected officials) are in a position to effect change   
2. Multi-item composite variable: The competency and responsiveness of fed, state, and business   
3. 2-item composite variable: The competency and responsiveness of ngo only
...
```{r, eval=T, echo=F}
d_pca_efficacyeffectchange <- d_accept %>% 
  dplyr::select(
    matches("sr_21a"),
    matches("sr_29[a-d]"))

psych::alpha(d_pca_efficacyeffectchange, use ="pairwise", na.rm = TRUE) 
```

#### Efficacy - 2nd Factor (Competency & Responsiveness of Fed, State, and Business) - Cronbach Alpha 
```{r, eval=T, echo=F}
d_pca_efficacycompetencyresponsive <- d_accept %>% 
  dplyr::select(
    matches("sr_27[a-c]"),
    matches("sr_28[a-c]"))

psych::alpha(d_pca_efficacycompetencyresponsive, use ="pairwise", na.rm = TRUE) 
```

#### Efficacy - 3rd Factor (Competency & Responsiveness of NGO) - Correlation 
```{r, eval=T, echo=F}
...
#### TPB Behavioral Attitude - 1st factor - Cronbach Alpha
```{r, eval=T, echo=F}
d_pca_behavioralattitude1 <- d_accept %>% 
  dplyr::select(
    matches("sr_34"),
    matches("sr_38"),
    matches("sr_36"),
...
psych::alpha(d_pca_behavioralattitude1, use ="pairwise", na.rm = TRUE) 
```
