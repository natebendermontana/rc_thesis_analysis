...
library(blorr)
blr_confusion_matrix(logit_final)
...
m1_log_predstest <- m1_log_predstest %>%
  mutate(term = c("CIM benefits", "Descriptive norms", "Age", "Perceived risk", "Efficacy: able to call", "Human ingenuity", "CC personal harm",  "CC harm future generations", "Injunctive norms"))
...
for_csv <- df3_clus_avg %>% 
  dplyr::select(age_true,
                ...
                sr_11_harm_future_generations_reversed,
                injunctcontactnorms_all_comp)
# write.csv(for_csv,"/Users/natebender/Desktop/repo/RCthesisanalysis/output_tables/meanscores_clusters.csv", row.names = TRUE)
...
p <- ggparcoord(
  ...
  groupColumn = "cluster", 
  scale = "globalminmax", # univariately, subtract mean and divide by standard deviation
  order = c(24, 6, 8, 10, 21, 11, 13, 12)) + 
...
library(purrr)
regression_clean2 %>%  # for categorical vars desc stats. List is in-progress, just experimenting at the moment. 
  select(
  ...
  split(regression_clean2$cluster) %>% 
  map(summary)

table(regression_clean$cluster)  # total cluster counts
table(regression_clean$cluster, regression_clean$sr_12a_actions_contacted_officials_binary)  # counts of contact by cluster

# categorical desc stats visualization

a <- ggplot(regression_clean2, aes(x=race_white_dumvar, fill=cluster))+
...
b <- ggplot(regression_clean2, aes(x=children_dumvar, fill=cluster))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('Percent of X') +
...
c <- ggplot(regression_clean2, aes(x=gender_dumvar, fill=cluster))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('Percent of X') +
...
d <- ggplot(regression_clean2, aes(x=sr_56_marital_status, fill=cluster))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('Percent of X') +
...
e <- ggplot(regression_clean2, aes(x=sr_61_education, fill=cluster))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('Percent of X') +
...
f <- ggplot(regression_clean2, aes(x=sr_71_employment_status, fill=cluster))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('Percent of X') +
...
g <- ggplot(regression_clean2, aes(x=sr_79_political_leaning, fill=cluster))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('Percent of X') +
...
h <- ggplot(regression_clean2, aes(x=sr_72_income, fill=cluster))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('Percent of X') +
...
i <- ggplot(regression_clean2, aes(x=sr_75_religion_dumvar, fill=cluster))+
  geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..]), position="dodge" ) +
  geom_text(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..], label=scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..]) ),
            stat="count", position=position_dodge(0.9), vjust=-0.5)+
  ylab('Percent of X') +
...
### exploring contact-only cluster means compared to overall means ####

regression_clean2 <- regression_clean %>% 
  dplyr::filter(sr_12a_actions_contacted_officials_binary == 1)

evs_forclustering <- regression_clean2 %>% 
  dplyr::select(age_true,
                cimbenefits_comp,
                desccontactnorms_all_comp,
                cimperceivedrisk_comp,
...
regression_clean2$cluster <- as.factor(kmean_3$cluster)

df3_clus_avg <- regression_clean2 %>%
  group_by(cluster) %>%
  summarize_if(is.numeric, mean)
...
overall_clus_avg <- regression_clean %>% 
  dplyr::select(age_true,
                cimbenefits_comp,
                desccontactnorms_all_comp,
                cimperceivedrisk_comp,
...
for_plot <- rbind(df3_clus_avg, overall_clus_avg)
range_1 <- function(x){   (x - min(x))/(max(x)-min(x)) * (5 - 0) + 0 }  # scaling age between 0 and 5 so it approximates the same range as the rest of the variables.
for_plot$age_true <- range_1(for_plot$age_true)

p <- ggparcoord(
  data = for_plot, 
  columns = c(2:10), # If I could find a way to call these by name instead of index that would be great
  groupColumn = "cluster", 
  scale = "globalminmax", # univariately, subtract mean and divide by standard deviation
  order = c(2, 7, 4, 10, 3, 6, 5, 9, 8)) + 
  labs(x = "Predictive variables", 
       y = "Mean value", 
       title = "Mean predictive variable scores by cluster") +
  theme(axis.text.x=element_text(angle=7, hjust=1))
