...
All explanatory variables were treated as continuous, except for the following categorical vars which were reduced to the following smaller number of factor levels in order to have enough responses per level for analysis. All of the below are treated as unordered except for Income and Education.  
...
## Attn John — Notes on modelling section
Based on my ordinal response variable (“how many times have you contacted elected officials re climate change in last 12 months”), it appears that fitting an ordinal logistic regression model is the preferential choice. I did that, but the model fails the Brant test of proportional odds, which as I understand it is a fundamental assumption of this approach and indicates that my data do not work for this approach. 

So, I’ve briefly looked at three other approaches: fitting a Multinomial Logistic Regression, something called a Generalized Ordered Logistic Regression, a Penalized Logistic Regression via Cyclic Coordinate Descent and finally a General Linear Model. 

I feel very lost in the weeds though for really understanding the applicability of these different approaches for my data, the different ways to test the assumptions & GOF for each approach, and even why the data does not appear to be applicable for the ordinal logistic regression approach in the first place. I’ve spent several hours trying to understand all this better, but can’t quite figure it out yet.

I believe because my data doesn't meet the proportional odds assumption, the preferred approach is then either using a multinomial logistic regression model, or a generalized ordered logistic regression. But I'm not sure. Any help seeing the forest for the trees here would be extremely appreciated! 
...
OLR_saturated <- polr(OLR_complete_cases$sr_12a_actions_contacted_officials ~ 
