...
Alex - per your suggestion I went back in and reduced several of the groups that were at four factors down to two factors. We lose a little bit in the amount of cumulative variance explained, but doing that seems to help the factors hang together even more reliably, and also had the side benefit of aligning all the factors for the descriptive and injunctive social norms: both now break down into two factors: "spouse & children" and all others: "parents, otherfamily, friends, coworkers, recreationpartners, faithcommunity"  
...
One factor solution (81% cumulative variance (hereafter: cv))   
1. All
...
pca1 <- fa(d_pca_descriptivecontactnorms, nfactors = 1, rotate = "Varimax", fm="minres") 
print(pca1)
print(pca1$loadings,cutoff = 0.3)

```

#### Descriptive Norms (contact) - 1st factor (Fam & non-fam) - Cronbach Alpha
...
# 11/23 START HERE WITH ROLE MODELS

### Descriptive Norms (role models)
*"How likely are you to consider each of these groups as behavioral role models?"*

Scree plot suggests four factors (78% cv), but two factor solution makes the most sense.   
Two factor solution (68% cv):   
1. 2-item composite variable: Spouse, Children   
2. Multi-item composite variable: parents, otherfam, friends, cowork, recpartners, faithcomm
...
pca1 <- fa(d_pca_descriptiverolemodelnorms, nfactors = 2, rotate = "Varimax", fm="minres") 
print(pca1)
print(pca1$loadings,cutoff = 0.3)
...
#### Descriptive Norms (role models) - 1st factor (Spouse & Children) - Correlation
```{r, eval=T, echo=F}
res <- cor.test(OLR_complete_cases$sr_20b_rolemodels_spouse,OLR_complete_cases$sr_20c_rolemodels_children, method = "spearman", exact=FALSE)
print(res)
```
...
#### Descriptive Norms (role models) - 2nd factor (parents, otherfam, friends, cowork, recpartners, faithcomm) - Cronbach Alpha
```{r, eval=T, echo=F}
d_pca_descriptivecontactnorms_other <- OLR_complete_cases %>% dplyr::select(
    matches("sr_20a_rolemodels_parents"),
    matches("sr_20d_rolemodels_otherfamily"),
    matches("sr_20e_rolemodels_friends"),
    matches("sr_20f_rolemodels_coworkers"),
...
psych::alpha(d_pca_descriptivecontactnorms_other, use ="pairwise") 
```
Alpha of .91 shows strong reliability, with no significant improvement found by dropping any items. 

```{r, eval=T, echo=F, include=F}
# Create two comp variables
OLR_complete_cases <- OLR_complete_cases %>% 
  mutate(
    descrolemodelnorms_spouse_child_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_20b_rolemodels_spouse, sr_20c_rolemodels_children))),
    descrolemodelnorms_other_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_20a_rolemodels_parents, sr_20d_rolemodels_otherfamily, sr_20e_rolemodels_friends, sr_20f_rolemodels_coworkers, sr_20g_rolemodels_recpartners, sr_20h_rolemodels_faithcommun))))
```
...
Scree plot suggests four factors (78% cv) but two (69% cv) makes the most sense.     
Two-factor solution:   
1. 2-item composite variable: Spouse, Children   
2. Multi-item composite variable: parents, otherfam, friends, cowork, recpartners, faithcomm

```{r, eval=T, echo=F}
d_pca_injunctivecontactnorms <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_22a"),
    matches("sr_22b"),
    matches("sr_22c"),
...
#### Injunctive Norms (contact) - 1st factor (Spouse & Children) - Correlation
```{r, eval=T, echo=F}
res <- cor.test(OLR_complete_cases$sr_22b_expect_contact_spouse,OLR_complete_cases$sr_22c_expect_contact_children, method = "spearman", exact=FALSE)
print(res)
```
Correlation test of the two descriptive norms items for Spouse and Children, showing moderate positive correlation. Spearman's rho is `r res$estimate` with a p-value of `r res$p.value`.

#### Injunctive Norms (contact) - 2nd factor (parents, otherfam, friends, cowork, recpartners, faithcomm) - Cronbach Alpha
```{r, eval=T, echo=F}
d_pca_injunctivecontactnorms_other <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_22a_expect_contact_parents"),
    matches("sr_22d"),
    matches("sr_22e"),
...
psych::alpha(d_pca_injunctivecontactnorms_other, use ="pairwise") 
```
.91 alpha shows high reliability. Dropping any items decreases the alpha. 

```{r, eval=T, echo=F, include=F}
# Create two comp variables
OLR_complete_cases <- OLR_complete_cases %>% 
  mutate(
    injunctcontactnorms_spouse_child_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_22b_expect_contact_spouse, sr_22c_expect_contact_children))),
    injunctcontactnorms_other_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_22a_expect_contact_parents, sr_22d_expect_contact_otherfamily, sr_22e_expect_contact_friends, sr_22f_expect_contact_coworkers, sr_22g_expect_contact_recpartners, sr_22h_expect_contact_faithcommun))))
```
$$\\[.05in]$$
### Injunctive Norms (motivation)
*"How motivated are you to comply with the expectations of each of these people?"*

Two factor solution (69% cv)    
1. Two-item composite variable: Spouse, Children   
2. Multi-item composite variable: parents, Otherfam, Friends, Cowork, Recpartners, Faith

```{r, eval=T, echo=F}
d_pca_injunctivemotivation <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_23a"),
    matches("sr_23b"),
    matches("sr_23c"),
...
fa.parallel(d_pca_injunctivemotivation, fm='minres', fa='fa')
pca1 <- fa(d_pca_injunctivemotivation, nfactors = 2, rotate = "Varimax", fm="minres") 
print(pca1)
print(pca1$loadings,cutoff = 0.3)
```

#### Injunctive Norms (motivation) - 1st factor (Spouse & children ) - Cronbach Alpha
```{r, eval=T, echo=F}
res <- cor.test(OLR_complete_cases$sr_23b_motivated_to_comply_spouse,OLR_complete_cases$sr_23c_motivated_to_comply_children, method = "spearman", exact=FALSE)
print(res)
```
Correlation test of the two injunctive motivation norms items for Spouse and Children, showing moderate positive correlation. Spearman's rho is `r res$estimate` with a p-value of `r res$p.value`.

#### Injunctive Norms (motivation) - 2nd factor (parents, otherfam, friends, cowork, recpartners, faithcomm) - Cronbach Alpha
```{r, eval=T, echo=F}
d_pca_injunctivemotivationnon_other <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_23a_motivated_to_comply_parents"),
    matches("sr_23d_motivated_to_comply_otherfamily"),
    matches("sr_23e_motivated_to_comply_friends"),
...
psych::alpha(d_pca_injunctivemotivationnon_other, use ="pairwise") 
```
.91 alpha shows strong reliabiity; no increase with dropping any items.  

```{r, eval=T, echo=F, include=F}
# Create two comp variables
OLR_complete_cases <- OLR_complete_cases %>% 
  mutate(
    injunctmotivation_spouse_child_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_23b_motivated_to_comply_spouse,sr_23c_motivated_to_comply_children))),
    injunctmotivation_other_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_23a_motivated_to_comply_parents, sr_23d_motivated_to_comply_otherfamily, sr_23e_motivated_to_comply_friends, sr_23f_motivated_to_comply_coworkers, sr_23g_motivated_to_comply_recpartners, sr_23h_motivated_to_comply_faithcommun))))
```
...
```{r, eval=T, echo=F}
d_pca_CIMbenefits <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_16"),
    matches("sr_17"),
    matches("sr_18"))
...
OLR_complete_cases <- OLR_complete_cases %>% 
  mutate(
    cimbenefits_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_16_hear_friends_talk, sr_17_hear_media, sr_18_hear_public_officials))))
```
...
```{r, eval=T, echo=F}
d_pca_CIMperceivedrisk <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_3b_importance_climatechange"),
    matches("sr_8_importance_personally"),
    matches("sr_9_worried"),
...
```{r, eval=T, echo=F}
d_pca_CIMperceivedriskpersonal <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_3b_importance_climatechange"),
    matches("sr_8_importance_personally"),
    matches("sr_9_worried"),
...
psych::alpha(d_pca_CIMperceivedriskpersonal, use ="pairwise") 
```
"#10 Harm you personally" loads onto the second factor slightly more than here in the first, but it's not by much, it makes intuitively more sense for it to be here in the first factor with the other more individually-focused questions, and it shows good reliability within this factor. 

#### CIM - Perceived Risk - 2nd factor - sr_11_harm_future_generations
```{r, eval=F, echo=F, include=F}
describe(OLR_complete_cases$sr_11_harm_future_generations)
```
...
OLR_complete_cases <- OLR_complete_cases %>% 
  mutate(
    cimperceivedrisk_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_3b_importance_climatechange, sr_8_importance_personally, sr_9_worried, sr_10_harm_you_personally))))
```
...
3. Multi-item composite variable: All types' (fed, state, business, ngo) competency and responsiveness   

```{r, eval=T, echo=F}
d_pca_efficacygroup <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_27[a-d]"),
    matches("sr_28[a-d]"),
    matches("sr_29[a-d]"))
...
```{r, eval=T, echo=F, include=F}
describe(OLR_complete_cases$sr_21a_effective_actions_contacting_officials)
```

#### Efficacy - 2nd Factor (All types' ability to effect change) - Cronbach Alpha 
```{r, eval=T, echo=F}
d_pca_efficacyeffectchange_all <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_29[a-d]"))
...
#### Efficacy - 3rd Factor (all types' competency & responsiveness) - Cronbach Alpha 
```{r, eval=T, echo=F}
d_pca_efficacycompetencyresponsive_all <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_27[a-d]"),
    matches("sr_28[a-d]"))
...
OLR_complete_cases <- OLR_complete_cases %>% 
  mutate(
    efficacy_effectiveness_all_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_29a_effective_fed, sr_29b_effective_state, sr_29c_effective_business, sr_29d_effective_ngo))),
    efficacy_competresp_all_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_27a_competent_fed, sr_27b_competent_state, sr_27c_competent_business, sr_27d_competent_ngo, sr_28a_responsive_fed, sr_28b_responsive_state, sr_28c_responsive_business, sr_28d_responsive_ngo))))
```
...
# PCA with original, non-reversed variables, if needed. 
# d_pca_behavioralattitude <- OLR_complete_cases %>% 
#   dplyr::select(
#     matches("sr_32"),
#     matches("sr_33"),
#     matches("sr_34"),
...
#     matches("sr_38"),
#     matches("sr_39"))
# summary(d_pca_behavioralattitude)
...
```{r, eval=T, echo=F}
res <- cor.test(OLR_complete_cases$sr_38_contact_admirable_reversed,OLR_complete_cases$sr_39_contact_good_reversed, method = "spearman")
print(res)
```
...
```{r, eval=T, echo=F}
d_pca_behavioralattitude2 <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_32_contact_useful"),
    matches("sr_33_contact_pleasant"),
    matches("sr_37_contact_sensible"))
...
```{r, eval=T, echo=F}
d_pca_behavioralattitude3 <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_34_contact_cool_reversed"),
    matches("sr_35_contact_easy_reversed"),
    matches("sr_36_contact_exciting_reversed"))
...
OLR_complete_cases <- OLR_complete_cases %>% 
  mutate(
    behatt_admirablegood_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_38_contact_admirable_reversed, sr_39_contact_good_reversed))),
    behatt_usefulpleasantsensible_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_32_contact_useful, sr_33_contact_pleasant, sr_37_contact_sensible))),
    behatt_coolexcitingeasy_comp = rowMeans(subset(OLR_complete_cases, select = c(sr_34_contact_cool_reversed, sr_35_contact_easy_reversed, sr_36_contact_exciting_reversed))))
```
...
```{r, eval=T, echo=F, warning=F}
res <- cor.test(OLR_complete_cases$sr_30_easy_to_call,OLR_complete_cases$sr_31_able_to_call, method = "spearman")
print(res)
...
```{r, eval=T, echo=F}
d_worldviews <- OLR_complete_cases %>% 
  dplyr::select(
    matches("sr_41a"),
    matches("sr_41b"),
    matches("sr_41c"),
...
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$race_white_dumvar)
# total responses by religion type
table(OLR_complete_cases$race_white_dumvar)
...
1) Number of responses: `r table(OLR_complete_cases$race_white_dumvar)[1]` white respondents and `r table(OLR_complete_cases$race_white_dumvar)[2]` other respondents. 
...
OLR_complete_cases$sr_7_believe_about_climate_change <- factor(OLR_complete_cases$sr_7_believe_about_climate_change, ordered=FALSE)
OLR_complete_cases$sr_7_believe_about_climate_change <- recode_factor(OLR_complete_cases$sr_7_believe_about_climate_change, "1"="notchanging", "2"="most_natural", "3"="natural_human", "4"="most_human", "5"="excl_human")
# total responses by religion type
table(OLR_complete_cases$sr_7_believe_about_climate_change)
```
...
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$gender_dumvar)
# total responses by religion type
table(OLR_complete_cases$gender_dumvar)
# proportion of total responses, grouped by religion option and level of response variable.
prop.table(table_1, margin=2)
```
...
```{r eval=T, echo=F, include=T}
# MARITAL STATUS
OLR_complete_cases$sr_56_marital_status <- factor(OLR_complete_cases$sr_56_marital_status, ordered=FALSE)
#levels(OLR_complete_cases$sr_56_marital_status)
# Breakdown of responses
table(OLR_complete_cases$sr_56_marital_status)
```
...
OLR_complete_cases$sr_56_marital_status <- recode_factor(OLR_complete_cases$sr_56_marital_status, "1"="single", "2"="married_partner", "3"="divorce_widow", "4"="divorce_widow")
# levels(OLR_complete_cases$sr_56_marital_status)
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$sr_56_marital_status)
# total responses by religion type
table(OLR_complete_cases$sr_56_marital_status)
# proportion of total responses, grouped by religion option and level of response variable.
prop.table(table_1, margin=2)
...
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$children_dumvar)
# total responses by religion type
table(OLR_complete_cases$children_dumvar)
# proportion of total responses, grouped by religion option and level of response variable.
prop.table(table_1, margin=2)

```
Totoal number of respondents by "children" and "no children" categories, as well as response proportions. 
...
```{r eval=T, echo=F, include=T}
# EDUCATION
OLR_complete_cases$sr_61_education <- factor(OLR_complete_cases$sr_61_education, ordered=TRUE)
# levels(OLR_complete_cases$sr_61_education)
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$sr_61_education)
# total responses by religion type
table(OLR_complete_cases$sr_61_education)
# proportion of total responses, grouped by religion option and level of response variable.
prop.table(table_1, margin=2)
```
...
OLR_complete_cases$sr_61_education <- recode_factor(OLR_complete_cases$sr_61_education, "1"="assocdeg_orlower", "2"="assocdeg_orlower", "3"="assocdeg_orlower", "4"="assocdeg_orlower", "5"="bachelordeg", "6"="graduatedeg")
# levels(OLR_complete_cases$sr_61_education)
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$sr_61_education)
# total responses by religion type
table(OLR_complete_cases$sr_61_education)
...
```{r eval=T, echo=F, include=T}
# EMPLOYMENT STATUS
OLR_complete_cases$sr_71_employment_status <- factor(OLR_complete_cases$sr_71_employment_status, ordered=FALSE)
# levels(OLR_complete_cases$sr_71_employment_status)
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$sr_71_employment_status)
# total responses by religion type
table(OLR_complete_cases$sr_71_employment_status)
# proportion of total responses, grouped by religion option and level of response variable.
prop.table(table_1, margin=2)
```
...
# all others -> "other"
OLR_complete_cases$sr_71_employment_status <- recode_factor(OLR_complete_cases$sr_71_employment_status, "1"="workfull", "2"="other", "3"="other", "4"="retired", "5"="other", "6"="other", "0"="other")
#levels(OLR_complete_cases$sr_71_employment_status)
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$sr_71_employment_status)
# total responses by religion type
table(OLR_complete_cases$sr_71_employment_status)
...
```{r eval=T, echo=F, include=T}
# INCOME
OLR_complete_cases$sr_72_income <- factor(OLR_complete_cases$sr_72_income, ordered=TRUE)
# levels(OLR_complete_cases$sr_72_income)
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$sr_72_income)
# total responses by religion type
table(OLR_complete_cases$sr_72_income)
# proportion of total responses, grouped by religion option and level of response variable.
prop.table(table_1, margin=2) 
```
...
# 100-150 -> stays as-is
# over 150 -> "over150k"
OLR_complete_cases$sr_72_income <- recode_factor(OLR_complete_cases$sr_72_income, "1"="under100k", "2"="under100k", "3"="under100k", "4"="under100k", "5"="100_150k", "6"="over150k", "7"="over150k", "8"="over150k", "9"="over150k")
...
```{r eval=T, echo=F, include=T}
# RELIGION
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$sr_75_religion_dumvar)
# total responses by religion type
table(OLR_complete_cases$sr_75_religion_dumvar)
# proportion of total responses, grouped by religion option and level of response variable.
prop.table(table_1, margin=2)
```
...
OLR_complete_cases$sr_78_political_affiliation <- factor(OLR_complete_cases$sr_78_political_affiliation, ordered=FALSE)
# levels(OLR_complete_cases$sr_78_political_affiliation)
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$sr_78_political_affiliation)
# total responses by religion type
table(OLR_complete_cases$sr_78_political_affiliation)
# proportion of total responses, grouped by religion option and level of response variable.
prop.table(table_1, margin=2)
```
...
# all others -> "other"
OLR_complete_cases$sr_78_political_affiliation <- recode_factor(OLR_complete_cases$sr_78_political_affiliation, "1"="democrat", "2"="republican", "3"="independent", "4"="other", "5"="other", "6"="other", "0"="other")
...
```{r eval=T, echo=F, include=T}
# POLITICAL IDEOLOGY
OLR_complete_cases$sr_79_political_leaning <- factor(OLR_complete_cases$sr_79_political_leaning, ordered=FALSE)
table_1 <- table(OLR_complete_cases$sr_12a_actions_contacted_officials, OLR_complete_cases$sr_79_political_leaning)
# total responses by religion type
table(OLR_complete_cases$sr_79_political_leaning)
# proportion of total responses, grouped by religion option and level of response variable.
prop.table(table_1, margin=2)
```
...
OLR_complete_cases$sr_79_political_leaning <- recode_factor(OLR_complete_cases$sr_79_political_leaning, "1"="liberal", "2"="liberal", "3"="moderate", "4"="conservative", "5"="conservative")
...
## Listwise Deletion
```{r listwise deletion eval=T, echo=F, include=F}
...
ord_reg_model <- polr(OLR_complete_cases$sr_12a_actions_contacted_officials ~ 
                        ...
                        descdynamicnorms_comp + desccontactnorms_spouse_child_comp +
                        desccontactnorms_other_comp + descrolemodelnorms_spouse_child_comp +
                        descrolemodelnorms_other_comp + injunctcontactnorms_spouse_child_comp +
                        injunctcontactnorms_other_comp + injunctmotivation_spouse_child_comp +
                        injunctmotivation_other_comp + cimbenefits_comp + 
...
Descriptive norms - contacting: 0.74
...
```
Looking at the absolute values of the coefficient tells us the most influential factors. So it looks like our top three, just to set a random cutoff, are respectively CIMperceivedrisk_comp (composite var of perceived risk), then CIMbenefits_comp (comp var of benefits as defined in the CIM), and finally the composite var of injunctive norms from the immediate family.
  
CIMperceivedrisk = sr_3b_importance_climatechange, sr_8_importance_personally, sr_9_worried, sr_10_harm_you_personally  
CIMbenefits = sr_16_hear_friends_talk, sr_17_hear_media, sr_18_hear_public_officials  
Injunctcontactnorms_fam_comp = sr_22b_expect_contact_spouse, sr_22c_expect_contact_children
...
```
Still wrapping my head around interpreting the output of OLR, but I think it's something like the following, just choosing one variable for demonstration:  
  
"For a one unit increase in composite variable of CIM Benefits, the odds of moving from "no contact" to "one contact" or "more than one contact" (in the last twelve months) are 1.92 times greater, given that the other variables in the model are held constant.”
