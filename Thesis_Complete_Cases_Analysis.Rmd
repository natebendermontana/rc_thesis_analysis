...
Scree plot suggests four factors, but one factor solution makes the most sense.   
One factor solution (65% cv):   
1. All
...
pca1 <- fa(d_pca_descriptiverolemodelnorms, nfactors = 1, rotate = "Varimax", fm="minres") 
print(pca1)
print(pca1$loadings,cutoff = 0.3)

```
#### Descriptive Norms (role models) - 1st factor (All) - Cronbach Alpha
```{r, eval=T, echo=F}
d_pca_descriptivecontactnorms_all <- d_accept %>% 
  dplyr::select(
    matches("sr_20a_rolemodels_parents"),
...
Scree plot suggests one factor (86% cv).
One factor solution: All
```{r, eval=T, echo=F}
...
pca1 <- fa(d_pca_injunctivecontactnorms, nfactors = 1, rotate = "Varimax", fm="minres") 
print(pca1)
print(pca1$loadings,cutoff = 0.3)
...
```{r, eval=T, echo=F}
d_pca_injunctivecontactnorms_all <- d_accept %>% 
  dplyr::select(
    matches("sr_22a_expect_contact_parents"),
    matches("sr_22b_expect_contact_spouse"),
    matches("sr_22c_expect_contact_children"),
...
psych::alpha(d_pca_injunctivecontactnorms_all, use ="pairwise") 
```
...
Plot suggests two factor solution (70% cv) but one factor (65% cv) makes sense and is reliable.     
1. All
```{r, eval=T, echo=F}
d_pca_injunctivemotivation <- d_accept %>% 
  dplyr::select(
    matches("sr_23a"),
    matches("sr_23b"),
    matches("sr_23c"),
...
pca1 <- fa(d_pca_injunctivemotivation, nfactors = 1, rotate = "Varimax", fm="minres") 
print(pca1)
print(pca1$loadings,cutoff = 0.3)
```
...
age
desc role model norms
sr_10 harm you personally
...
For a one unit increase in composite variable of CIM Benefits, the odds of moving from "no contact" to "one contact" or "more than one contact" (in the last twelve months) are 2.17 times greater, given that the other variables in the model are held constant.
...
Descriptive Contact Norms, CIM Benefits, Perceived Risk, Injunctive Contact Norms all have a positive effect on propensity to contact officials. The more a person perceives positive descriptive norms related to contacting officials; perceives positive injunctive norms related to contacting officials; hears about CC from friends, media, & public figures; or perceives CC as a risk, the more likely they are to contact officials. 

Conversely, the more a person believes CC will harm future generations or the more they agree with the statement that "human ingenuity will ensure we do not make the Earth unlivable", the less likely they are to contact officials. 
...
ggpairs(OLR_complete_cases[, c(2:6)], title = "Correlation Plot between each Variable")

# all other continuous factors/variables
ggpairs(OLR_complete_cases[, c(7:13)], title = "Correlation Plot between each Variable")
