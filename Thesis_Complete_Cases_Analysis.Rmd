...
m1_log_predstest <- m1_log_predstest %>%
 mutate(term = c("Descriptive contact norms", "Injunctive contact norms", "CIM benefits", "Behavioral attitude: cool, exciting, easy", "Efficacy: able to call", "Agreement: humans have right to modify world" ))
...
evs_forclustering <- regression_clean %>% 
  select(desccontactnorms_all_comp, injunctcontactnorms_all_comp, cimbenefits_comp, behatt_coolexcitingeasy_comp, sr_31_able_to_call, sr_41a_right_to_modify)
...
library(cluster)
gap_stat <- clusGap(evs_forclustering, FUN = kmeans, nstart = 1,
                    K.max = 10, B = 20)
...
```{r}
# Add clusters back in for both 2-cluster and 3-cluster solutions. Do some descriptive stats and see which approach is more useful. 



# Now that the two dataframes are same length, add cluster numbers to orig dataframe
nike_sub_complete$cluster3 <- kmean_3$cluster

# alt approach using cbind
regression_clean_clust3 <- cbind(regression_clean, cluster = kmean_3$cluster)


# Rename clusters from numbers to names
```
